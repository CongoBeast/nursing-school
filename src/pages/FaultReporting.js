...
const FaultReporting = () => { // Set to "admin" to see enabled buttons
  const [showModal, setShowModal] = useState(false);

  const userRole = localStorage.userType
  ...
  return (
    <div style={{ backgroundColor: '#E3F2FD', minHeight: '100vh', padding: '40px 20px' }}>
      <div className="container" style={{ maxWidth: '1000px' }}>
        
        {/* Header Section */}
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 style={{ color: '#1565C0', fontWeight: 'bold' }}>
              ...
              <tbody>
                {faultRecords.map((fault) => (
                  <tr key={fault.id} className="align-middle">
                    ...
                    <td>{fault.date}</td>
                    <td>
                      <small className="fw-bold" style={{ color: fault.status === 'Fixed' ? '#28a745' : '#42A5F5' }}>
                        {fault.status}
                      </small>
                    </td>
                    <td>
                      <button 
                        ...
                        style={{ backgroundColor: '#90CAF9', color: '#1565C0' }}
          ...
          <div className="modal d-block" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content" style={{ borderRadius: '15px', border: 'none' }}>
                <div className="modal-header" style={{ backgroundColor: '#1565C0', color: 'white' }}>
                  ...
                  <form>
                    <div className="row">
                      <div className="col-md-6 mb-3">
                        <label className="form-label fw-bold" style={{ color: '#1565C0' }}>Building</label>
                      ...
                      <div className="col-md-6 mb-3">
                        <label className="form-label fw-bold" style={{ color: '#1565C0' }}>Dorm Number</label>
                        <input type="text" className="form-control" placeholder="e.g. A12" required />
                      </div>
                    </div>

                    <div className="mb-3">
                      <label className="form-label fw-bold" style={{ color: '#1565C0' }}>Item Broken</label>
                      <input type="text" className="form-control" placeholder="e.g. Ceiling Fan" required />
                    </div>

                    <div className="mb-3">
                      <label className="form-label fw-bold" style={{ color: '#1565C0' }}>Details</label>
                      <textarea className="form-control" rows="3" placeholder="Describe the issue..."></textarea>
                    </div>

                    <div className="mb-3">
                      <label className="form-label fw-bold" style={{ color: '#1565C0' }}>Upload Image</label>
                    ...
                    <div className="mb-4">
                      <label className="form-label fw-bold" style={{ color: '#1565C0' }}>Discovery Date</label>
                      <input type="date" className="form-control" defaultValue={new Date().toISOString().split('T')[0]} />
                    </div>

                    <button type="submit" className="btn w-100 py-2 fw-bold" style={{ backgroundColor: '#1565C0', color: 'white' }}>
                      Submit Report
                    </button>
                  </form>
                </div>
